#!/bin/sh
# Builds the DOSBox-X waterbox core package and installs it into a chimera
# checkout as build/Cores/dosbox-x.chimeraCore.
#
...
dosbox_pin="$(git -C "$root/extern/dosbox-x" describe --tags --always 2>/dev/null || echo unknown)"
...
json.dump({
    ...
    "toolchain": {"compiler": "gcc $gccver", "libstdc++": "$gccver",
                  "binutils": "$binutils_version", "target": "x86_64-linux-musl",
                  "musl": "$musl_version"},
    "guestKit": {"name": "miniBox", "commit": "$guest_kit"},
    "upstream": {"name": "DOSBox-X", "pin": "$dosbox_pin"},
    "builtOn": "$os_id",
...
mkdir -p "$cores_dir"
zip_path="$cores_dir/dosbox-x.chimeraCore"
rm -f "$zip_path"
...
for cache in "$chimera_root"/build/CoreCache/dosbox-x-*; do
