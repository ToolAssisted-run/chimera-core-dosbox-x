...
/* Every field here is a DECLARED SETTING (waterbox.config, generated by
 * gen-config.py), and the default of every field is that setting's declared
 * default. There is no configuration preset: the wizard's presets are a list of
 * VALUES it writes into these settings and is then finished with, so by the
 * time the core sees a machine it is nothing but settings. See
 * chimera docs/project.md, "Configuration presets". */
struct DosDrvMachine {
	bool joystick1 = true;   // plugged into gameport 1
	bool joystick2 = true;   // plugged into gameport 2
	// ---- the machine: what the ten declared presets write ------------------
	std::string videoCardType = "svga_s3";  // conf [dosbox] machine
	int32_t memsizeMB = 32;                 // conf [dosbox] memsize
	int32_t memsizeKB = 0;                  // conf [dosbox] memsizekb (ADDED to memsize)
	std::string cpuType = "486";            // conf [cpu] cputype
	int32_t cpuCycles = 22000;              // conf [cpu] cycles, always "fixed N"
	std::string cpuCore = "normal";         // conf [cpu] core
	std::string soundBlasterModel = "sbpro2"; // conf [sblaster] sbtype
	int32_t videoMemoryMB = -1;             // conf [video] vmemsize
	int32_t vesaWidthLimit = 1280;          // conf [video] vesa modelist width limit
	int32_t vesaHeightLimit = 1024;         // conf [video] vesa modelist height limit
	std::string dosVersion = "auto";        // conf [dos] ver, omitted when "auto"
	int32_t hardDriveDataRate = -1;         // conf [dos] hard drive data rate limit
	int32_t floppyDriveDataRate = -1;       // conf [dos] floppy drive data rate limit
	bool int13FakeIo = false;               // the five int13fake* keys, [ide, *] + [fdc, primary]
	int32_t cdromInsertionDelayMs = 0;      // conf [ide, secondary] cd-rom insertion delay
	// ---- the rest of the machine, which no preset moves --------------------
	std::string pcSpeaker = "enabled";      // conf [speaker] pcspeaker
	int32_t soundBlasterIRQ = -1;           // conf [sblaster] irq when not -1
	...
	std::string midiDevice = "none"; // "mt32_old"/"mt32_new": MT32_*.ROM; "cm32l": CM32L_*.ROM
...
// The composed dosbox-x.conf: base conf + the settings' sections + an autoexec
// mounting the loaded file ("rom") and the hard disk. Nothing else: a machine
...
// NAME=VALUE; both land here, so the native reference and the sandbox cannot
// drift over what a setting means. false = no setting of that name.
