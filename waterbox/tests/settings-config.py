...
if len(sys.argv) > 4:
    ...
    for decl_id, path in json.loads(sys.argv[4]).items():
        fw["DOSBox-X/" + decl_id] = path
# Pick this core explicitly: a miniHawk install can hold several packages claiming the DOS, and --core only LOADS a package, it does not choose it.
cfg.setdefault("DefaultCores", {})["DOS"] = "DOSBox-X"
