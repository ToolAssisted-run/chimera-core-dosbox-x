...
private:
	...
	bool load_rom_set(std::string romDir, int model) {
		/* chimera: the sandbox's namespace is flat and matched exactly - "./NAME"
		 * is not NAME there - so the work directory is asked for by bare name. */
		if (romDir == "./") romDir.clear();
		if (!romDir.empty() && romDir.back() != '/' && romDir.back() != '\\') {
			romDir.append("/");
		}
		char pathName[4096];
		std::string roms = "";
