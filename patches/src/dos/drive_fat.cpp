...
extern bool _floppyDriveUsed;
/* chimera: a floppy is not the hard disk. One flag lit the "Hard Disk" light for
 * either, and a machine with both could not say which was touched. */
static inline void chimera_drive_used(fatDrive *drive);
...
fatDrive::fatDrive(const char* sysFilename, uint32_t bytesector, uint32_t cylsector, uint32_t headscyl, uint32_t cylinders, std::vector<std::string>& options) {
	...
	if (!diskfile) {
		...
		unsigned char membootcode[256];
		jaffarCommon::file::MemoryFile::fseeko64(memfile, 0L, SEEK_SET);
		if (jaffarCommon::file::MemoryFile::fread(membootcode, sizeof(membootcode), 1, memfile) != 1) {
			LOG(LOG_IO, LOG_ERROR)("Reading error in fatDrive constructor\n");
			created_successfully = false;
			return;
		}

		const char *mext = strrchr(sysFilename, '.');
		const bool mem_is_hdd = (mext != NULL) && (!strcasecmp(mext, ".hdi") || !strcasecmp(mext, ".nhd"));
		jaffarCommon::file::MemoryFile::fseeko64(memfile, 0L, SEEK_END);
		const uint32_t memfilesize = (uint32_t)(jaffarCommon::file::MemoryFile::ftello64(memfile) / 1024L);
		loadedDisk = new imageDisk_Mem(memfile, fname, memfilesize, (mem_is_hdd | (memfilesize > 2880)));
		fatDriveInit(sysFilename, bytesector, cylsector, headscyl, cylinders, memfilesize, options);
		return;
	}
...
static inline void chimera_drive_used(fatDrive *drive) {
	if (drive != NULL && drive->loadedDisk != NULL && !drive->loadedDisk->hardDrive) _floppyDriveUsed = true;
	else _diskDriveUsed = true;
}
