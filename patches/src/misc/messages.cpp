...
{
    /* wbx: thread_local compiles to a %fs-relative access, which the sandbox
     * does not virtualize (host TLS on Linux, fs base 0 on Windows). Green
     * threads run one at a time, so a plain static carries the same value. */
