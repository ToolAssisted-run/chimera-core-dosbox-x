...
/* wbx: dosbox-x keeps ceil_udivide in src/dos/cdrom.h, but these decoders
 * (imported from DOSBox Staging) expect it from support.h; define it here. */
#ifndef WBX_CEIL_UDIVIDE
#define WBX_CEIL_UDIVIDE
template <typename T1, typename T2>
inline constexpr T1 wbx_ceil_udivide_helper(const T1 x, const T2 y) noexcept {
    return (T1)((x != 0) ? 1 + ((x - 1) / y) : 0);
}
#ifndef ceil_udivide
#define ceil_udivide wbx_ceil_udivide_helper
#endif
#endif
