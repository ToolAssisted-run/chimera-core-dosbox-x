...
/* chimera: the free fonts a desktop install keeps beside the executable
 * travel inside the core, which has no disk to keep them on. Asked only after
 * the working directory, so a file the user supplied still wins. */
extern "C" FILE *chimera_bundled_file(const char *name) __attribute__((weak));
...
FILE *Try_Load_FontFile(std::string filename) {
    ...
    if (chimera_bundled_file && (fp = chimera_bundled_file(filename.c_str())))
        return fp;
