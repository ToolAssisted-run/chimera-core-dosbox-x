...
/* wbx: A CORE HAS NO MENU BAR. Every other build of DOSBox-X is an application
 * with a window somebody clicks in; this one is a machine behind an ABI, drawn
 * into a buffer a frontend owns, with no user to open a menu and nowhere to put
 * one. Compiling the SDL-drawn menus in anyway is not merely wasted: they are
 * laid out on every video mode change (output_surface.cpp calls setScale, which
 * calls layoutMenu), and that walks a display list of item handles into a
 * master list a headless core never populated - "DOSBoxMenu::get_item()
 * attempt to get out of range handle", E_Exit, and the machine is gone. Which
 * is what a DOS game switching to 640x480 did. */
#if defined(CHIMERA_WATERBOX)
# define DOSBOXMENU_TYPE    DOSBOXMENU_NULL
#elif C_FORCE_MENU_SDLDRAW /* Programmer/Dev wants to compile with SDL drawn menus even if host OS offers menus (shrug) Ok */
